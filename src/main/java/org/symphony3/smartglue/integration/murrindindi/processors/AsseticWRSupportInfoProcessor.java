...
public class AsseticWRSupportInfoProcessor implements Runnable{
    private final String trackId;
    private final String authorityToken;
    List<Future<?>> futures = new ArrayList<>();
    private final AuthorityTaskManagingData taskData;
    private static final Logger log = LogManager.getLogger(AsseticWRSupportInfoProcessor.class);
    public AsseticWRSupportInfoProcessor(String trackId, String authorityToken, AsseticWorkRequestByExternalIdResponse.Resource wr,
        ...
        taskData.setCode(MurrindindiConstants.AUTHORITY_SUPPORT_INFO_TASK);
        taskData.setTaskNote("Operations & Maintenance additional information - "+info);
        taskData.setAuthorityToken(authorityToken);
        taskData.setPrefix(MurrindindiConstants.DYNAMO_WR_SUPPORT_INFO_PREFIX);
    ...
    @Override
    public void run() {
        log.info("{} starting AsseticWRSupportInfoProcessor for: {}",trackId,taskData.getFormattedAccount());
        QueryResponse response = DynamoManager.getEntryFromDynamo(MurrindindiConstants.DYNAMO_WR_SUPPORT_INFO_PREFIX+
        ...
        if (tasks != null && !tasks.isEmpty()) {
            ...
            for (AuthorityTaskResponse.Result task : tasks) {
                if (MurrindindiConstants.AUTHORITY_SUPPORT_INFO_TASK.equals(task.getWorkflowActionCode())) {
            ...
            if (!taskFound) {
                ...
                if (taskData.isRetry()) {
                    DynamoManager.updateEntryOnDynamo(taskData.getPrefix()+taskData.getFormattedAccount()+"/"+taskData.getInfoId(),MurrindindiConstants.DYNAMO_STATUS_SUCCESS," ",
