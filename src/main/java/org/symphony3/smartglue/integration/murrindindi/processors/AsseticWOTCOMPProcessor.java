...
public class AsseticWOTCOMPProcessor implements Runnable{
    ...
    public AsseticWOTCOMPProcessor(String trackId, String authorityToken, AsseticGetWorkOrderByTimeResponse.WorkOrder wo) {
        ...
        taskData.setCode(MurrindindiConstants.AUTHORITY_INVESTIGATE_TASK);
        taskData.setTaskNote("Works completed");
        taskData.setAuthorityToken(authorityToken);
        taskData.setPrefix(MurrindindiConstants.DYNAMO_WO_TCOMP_PREFIX);
        ...
        if (tasks != null && !tasks.isEmpty()) {
            ...
            if (!taskFound) {
                log.info("{} task {} is not present in the authority. Hence integration will not " +
                        "proceed further: {}",trackId,MurrindindiConstants.AUTHORITY_INVESTIGATE_TASK,taskData.getFormattedAccount());
            } else {
                log.info("{} task {} is present in the authority. Hence integration will " +
                        "proceed further: {}",trackId,MurrindindiConstants.AUTHORITY_INVESTIGATE_TASK,taskData.getFormattedAccount());
                String note = AuthorityManager.getTaskNotes(trackId,taskData.getTaskId(),taskData.getAuthorityToken(),MurrindindiConstants.TENANT_INTEGRATION_ID_ASSETIC_TO_CIVICA_MURRINDINDI, taskData.getFriendlyIdStr(),taskData.getFormattedAccount());
