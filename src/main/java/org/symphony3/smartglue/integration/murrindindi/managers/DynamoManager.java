...
import org.symphony3.smartglue.integration.murrindindi.operators.DynamoOperator;
...
public class DynamoManager {
    ...
    public static QueryResponse getEntryFromDynamo(String sourceId, String trackId){
        try {
            ...
            String partitionKeyName = MurrindindiConstants.AWS_DYNAMO_INTEGRATION_RECORD_TABLE_PARTITION_KEY1;
            String sourceIdKey = MurrindindiConstants.AWS_DYNAMO_INTEGRATION_RECORD_TABLE_PARTITION_KEY2;
            String partitionKeyValue = MurrindindiConstants.TENANT_INTEGRATION_ID_MURRINDINDI; // Partition key value
    ...
    public static QueryResponse getFailedEntryFromDynamo(String trackId){
        try {
            ...
            expressionAttributeValues.put(":failed", AttributeValue.builder().s(MurrindindiConstants.DYNAMO_STATUS_FAILED).build());
            expressionAttributeValues.put(":failed_actn", AttributeValue.builder().s(MurrindindiConstants.DYNAMO_STATUS_FAILED_ACTN_TASK_NOTES).build());
            expressionAttributeValues.put(":received", AttributeValue.builder().s(MurrindindiConstants.DYNAMO_STATUS_RECEIVED).build());
            expressionAttributeValues.put(":" + partitionKeyName, AttributeValue.builder().s(partitionKeyValue).build());
    ...
    public static void createEntryOnDynamo(String sourceId, String trackId) {
        try {
            ...
            data.setTargetId("");
            data.setStatus(MurrindindiConstants.DYNAMO_STATUS_RECEIVED);
    ...
    public static QueryResponse getRecentSuccessEntriesFromDynamo(String trackId) {
        try {
            String tableName = MurrindindiConstants.AWS_DYNAMO_INTEGRATION_RECORD_TABLE_NAME;
            String partitionKeyName = MurrindindiConstants.AWS_DYNAMO_INTEGRATION_RECORD_TABLE_PARTITION_KEY1;
            String partitionKeyValue = MurrindindiConstants.TENANT_INTEGRATION_ID_MURRINDINDI;
            ...
            Map<String, AttributeValue> expressionAttributeValues = new HashMap<>();
            expressionAttributeValues.put(":success", AttributeValue.builder().s(MurrindindiConstants.DYNAMO_STATUS_SUCCESS).build());
