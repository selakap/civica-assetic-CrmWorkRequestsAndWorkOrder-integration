...
public class AuthorityTaskManager {
    ...
    public static String authorityTaskOpen(String trackId, AuthorityTaskManagingData taskData) {
        ...
        try {
            ...
            HttpRsponseData res = AuthorityManager.openTask(trackId,payload,taskData.getAuthorityToken(),MurrindindiConstants.TENANT_INTEGRATION_ID_ASSETIC_TO_CIVICA_MURRINDINDI,taskData);
    ...
    public static boolean addTaskNotes(String trackId, AuthorityTaskManagingData taskData) {
        ...
        } else if (MurrindindiConstants.DYNAMO_WO_TCOMP_PREFIX.equals(taskData.getPrefix())) {
        ...
        HttpRsponseData res = AuthorityManager.updateTaskNotes(trackId,taskData.getTaskId(),taskData.getAuthorityToken(),encodedNote,MurrindindiConstants.TENANT_INTEGRATION_ID_ASSETIC_TO_CIVICA_MURRINDINDI,taskData.getFriendlyIdStr(),taskData.getFormattedAccount());
        if (res.getStatusCode() == Integer.parseInt(MurrindindiConstants.STATUS_CODE_200)) {
    ...
    public static void taskClose(String trackId, AuthorityTaskManagingData taskData) {
        ...
        try {
            ...
            HttpRsponseData res = AuthorityManager.closeTask(trackId,taskData.getTaskId(),
                    payload,taskData.getAuthorityToken(),MurrindindiConstants.TENANT_INTEGRATION_ID_ASSETIC_TO_CIVICA_MURRINDINDI,taskData);
            if (res.getStatusCode() == Integer.parseInt(MurrindindiConstants.STATUS_CODE_200)) {
    ...
    public static AuthorityTaskCloseRequest getTaskClosePayload(boolean isACTN) {
        ...
        if (isACTN) {
            req.setDeterminationCode(MurrindindiConstants.AUTHORITY_TASK_CLOSE_CODE_ACTN);
        } else {
            req.setDeterminationCode(MurrindindiConstants.AUTHORITY_TASK_CLOSE_CODE);
        }
    ...
    public static void dynamoStatusUpdate(String trackId, boolean isFailed, AuthorityTaskManagingData taskData) {
        ...
        if (isFailed) {
            if (isRetry) {
                DynamoManager.updateEntryOnDynamo(sourceId,MurrindindiConstants.DYNAMO_STATUS_FAILED," ",
                        message,trackId);
            } else {
                createDynamoEntry(trackId,sourceId, MurrindindiConstants.DYNAMO_STATUS_FAILED,
                        formattedAccount, message);
            }
        } else if (isEnd) {
            if (!isRetry) {
                createDynamoEntry(trackId,sourceId, MurrindindiConstants.DYNAMO_STATUS_SUCCESS,
                        formattedAccount, message);
            } else {
                DynamoManager.updateEntryOnDynamo(sourceId,MurrindindiConstants.DYNAMO_STATUS_SUCCESS," ",
                        message,trackId);
            }
            IntegrationCompletedEventData items = new IntegrationCompletedEventData(Integer.parseInt(MurrindindiConstants.COMPLETED_EVENT_RECORD_TYPE_RECORD),
                    sourceId, taskData.getTaskId()+"/"+taskData.getFormattedAccount(), MurrindindiConstants.COMPLETED_EVENT_SUCCESS,
                    1, MurrindindiConstants.STATUS_CODE_200, message,Integer.parseInt(MurrindindiConstants.TENANT_INTEGRATION_ID_ASSETIC_TO_CIVICA_MURRINDINDI));
    ...
    public static void createDynamoEntry(String trackId, String sourceId, String status, String targetId, String payload) {
        ...
        data.setTenantIntegrationsId(MurrindindiConstants.TENANT_INTEGRATION_ID_MURRINDINDI);
