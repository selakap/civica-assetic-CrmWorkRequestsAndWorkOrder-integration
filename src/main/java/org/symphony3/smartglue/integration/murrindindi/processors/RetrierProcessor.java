...
public class RetrierProcessor implements Runnable{
            ...
            for (Map<String, AttributeValue> result: response.items()){
                ...
                if (!isWithinLast3Minutes(data.getUpdatedAt())) {
                    if (data.getStatus().startsWith("failed") || "received".equals(data.getStatus())) {
                        ...
                        } else if (data.getSourceId().startsWith(MurrindindiConstants.DYNAMO_WR_INSPECT_PREFIX)) {
                            failedAsseticWRInspectRetry(data); //wrInspect Retries
                        } else if (data.getSourceId().startsWith(MurrindindiConstants.DYNAMO_WR_REJECT_PREFIX)) {
                            failedAsseticWRRejectRetry(data); //wrReject Retries
                        } else if (data.getSourceId().startsWith(MurrindindiConstants.DYNAMO_WR_SUPPORT_INFO_PREFIX)) {
                            failedAsseticWRSupportInfoRetry(data); //wrSupportInfo Retries
                        } else if (data.getSourceId().startsWith(MurrindindiConstants.DYNAMO_WO_NEW_PREFIX)) {
                            failedAsseticWONewRetry(data); //woNew Retries
                        } else if (data.getSourceId().startsWith(MurrindindiConstants.DYNAMO_WO_INPRG_PREFIX)) {
                            failedAsseticWOINPRGRetry(data); //woNew Retries
                        } else if (data.getSourceId().startsWith(MurrindindiConstants.DYNAMO_WO_TCOMP_PREFIX)) {
                            failedAsseticWOTCOMPRetry(data); //woNew Retries
                        } else if (data.getSourceId().startsWith(MurrindindiConstants.DYNAMO_WO_SUPPORT_INFO_PREFIX)) {
    ...
    public void failedAuthorityTaskEntriesRetry(DynamoData data) {
        ...
        AuthorityTaskResponse.Result task = AuthorityManager.getCrmTaskById(trackId, authorityToken, getTaskId(data.getSourceId()),MurrindindiConstants.TENANT_INTEGRATION_ID_MURRINDINDI,data.getSourceId());
        AuthorityCrmResponse.ResultItem crm = AuthorityManager.getCrmByFormattedAccount(trackId, getFormattedAccount(data.getSourceId()), authorityToken, MurrindindiConstants.TENANT_INTEGRATION_ID_MURRINDINDI);
        if (MurrindindiConstants.DYNAMO_STATUS_RECEIVED.equals(data.getStatus()) ||
    ...
    public void failedAsseticWRInspectRetry(DynamoData data) {
        ...
        data.setRetriable(true);
        String formattedAccount = data.getSourceId().substring(MurrindindiConstants.DYNAMO_WR_INSPECT_PREFIX.length());
        AsseticWorkRequestByExternalIdResponse.Resource wr = AsseticManager.getWorkRequestByFilter(trackId,formattedAccount,MurrindindiConstants.ASSETIC_EXTERNAL_IDENTIFIER_FILTER,MurrindindiConstants.TENANT_INTEGRATION_ID_ASSETIC_TO_CIVICA_MURRINDINDI," ");
        if (wr != null) {
            String executionId = trackId+"-"+ UUID.randomUUID().toString().replace("-", "").substring(0, 10);
            ...
            DynamoManager.updateEntryOnDynamo(data.getSourceId(),MurrindindiConstants.DYNAMO_STATUS_FAILED," ",
                    " ",trackId);
        }
    }
    ...
    public void failedAsseticWRRejectRetry(DynamoData data) {
        ...
        data.setRetriable(true);
        String formattedAccount = data.getSourceId().substring(MurrindindiConstants.DYNAMO_WR_REJECT_PREFIX.length());
        AsseticWorkRequestByExternalIdResponse.Resource wr = AsseticManager.getWorkRequestByFilter(trackId,formattedAccount,MurrindindiConstants.ASSETIC_EXTERNAL_IDENTIFIER_FILTER,MurrindindiConstants.TENANT_INTEGRATION_ID_ASSETIC_TO_CIVICA_MURRINDINDI," ");
        if (wr != null) {
            String executionId = trackId+"-"+ UUID.randomUUID().toString().replace("-", "").substring(0, 10);
    ...
    public void failedAsseticWRSupportInfoRetry(DynamoData data) {
        ...
        data.setRetriable(true);
        String sourceId = data.getSourceId().substring(MurrindindiConstants.DYNAMO_WR_SUPPORT_INFO_PREFIX.length());
    ...
    public void failedAsseticWONewRetry(DynamoData data) {
        ...
        data.setRetriable(true);
        String formattedAccount = data.getSourceId().substring(MurrindindiConstants.DYNAMO_WO_NEW_PREFIX.length());
        AsseticGetWorkOrderByTimeResponse.WorkOrder wo = AsseticManager.getWorkOrderByExternalId(trackId,formattedAccount,MurrindindiConstants.TENANT_INTEGRATION_ID_ASSETIC_TO_CIVICA_MURRINDINDI," ");
        if (wo != null) {
            String executionId = trackId+"-"+ UUID.randomUUID().toString().replace("-", "").substring(0, 10);
    ...
    public void failedAsseticWOINPRGRetry(DynamoData data) {
        ...
        data.setRetriable(true);
        String formattedAccount = data.getSourceId().substring(MurrindindiConstants.DYNAMO_WO_INPRG_PREFIX.length());
        AsseticGetWorkOrderByTimeResponse.WorkOrder wo = AsseticManager.getWorkOrderByExternalId(trackId,formattedAccount,MurrindindiConstants.TENANT_INTEGRATION_ID_ASSETIC_TO_CIVICA_MURRINDINDI, "");
        if (wo != null) {
            String executionId = trackId+"-"+ UUID.randomUUID().toString().replace("-", "").substring(0, 10);
    ...
    public void failedAsseticWOTCOMPRetry(DynamoData data) {
        ...
        data.setRetriable(true);
        String formattedAccount = data.getSourceId().substring(MurrindindiConstants.DYNAMO_WO_TCOMP_PREFIX.length());
        AsseticGetWorkOrderByTimeResponse.WorkOrder wo = AsseticManager.getWorkOrderByExternalId(trackId,formattedAccount,MurrindindiConstants.TENANT_INTEGRATION_ID_ASSETIC_TO_CIVICA_MURRINDINDI," ");
        if (wo != null) {
            String executionId = trackId+"-"+ UUID.randomUUID().toString().replace("-", "").substring(0, 10);
    ...
    public void failedAsseticWOSupportInfoRetry(DynamoData data) {
        ...
        data.setRetriable(true);
        String sourceId = data.getSourceId().substring(MurrindindiConstants.DYNAMO_WO_SUPPORT_INFO_PREFIX.length());
        String formattedAccount = sourceId.split("/")[0];
        String infoId = sourceId.split("/")[1];
        AsseticGetWorkOrderByTimeResponse.WorkOrder wo = AsseticManager.getWorkOrderByExternalId(trackId,formattedAccount,MurrindindiConstants.TENANT_INTEGRATION_ID_ASSETIC_TO_CIVICA_MURRINDINDI," ");
