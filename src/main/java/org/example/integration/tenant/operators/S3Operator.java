...
public class S3Operator {
    ...
    public static String getLastRuntime(String trackId, String fileName) throws IOException {
        ...
        S3Client s3Client = S3Client.builder()
                .region(Region.of(TenantConstants.AWS_REGION))
                .credentialsProvider(StaticCredentialsProvider.create(AwsBasicCredentials.create(TenantConstants.AWS_ACCESS_KEY_INTEGRATION,
                        TenantConstants.AWS_SECRET_KEY_INTEGRATION)))
                .build();
        // Construct the path to the JSON file
        String filePath = TenantConstants.AWS_TENANT_FOLDER + TenantConstants.SLASH+
                TenantConstants.TENANT_ID + TenantConstants.SLASH
                + TenantConstants.AWS_LAST_RUN_TIME_FOLDER +TenantConstants.SLASH+fileName;
        log.info("{} Fetching file from S3: {}",trackId, filePath);

        try {
            // Get the JSON file from S3
            GetObjectRequest getObjectRequest = GetObjectRequest.builder()
                    .bucket(TenantConstants.AWS_BUCKET_NAME)
                    .key(filePath)
                    .build();
    ...
    public static List<AuthorityTaskConfiguration.AuthorityTaskConfig> loadConfigFile() {
        ...
        String bucketName = TenantConstants.AUTHORITY_CONFIG_BUCKET_NAME;
        String objectKey = TenantConstants.AUTHORITY_CONFIG_FILE_NAME;
        ...
        try {
            S3Client s3Client = S3Client.builder()
                    ...
                    .credentialsProvider(StaticCredentialsProvider.create(AwsBasicCredentials.create(
                            TenantConstants.AWS_ACCESS_KEY_INTEGRATION, TenantConstants.AWS_SECRET_KEY_INTEGRATION)))
                    .build();
            GetObjectRequest getObjectRequest = GetObjectRequest.builder()
                    .bucket(bucketName)
                    .key(key)
