...
public class AsseticMainWRsProcessor implements  Runnable{
    ...
    public void run() {
        try {
            ...
            String lastRunTime = S3Operator.getLastRuntime(trackId, TenantConstants.ASSETIC_WR_LAST_RUNTIME);
    ...
    public void getWRList(String lastRunTime) {
        ...
        List<AsseticWorkRequestByExternalIdResponse.Resource> wrList = AsseticManager.getWorkRequestByLastModifiedTime(trackId,lastRunTime,nextRunTime,TenantConstants.TENANT_INTEGRATION_ID_ASSETIC_TO_CIVICA);
        if (wrList != null) {
