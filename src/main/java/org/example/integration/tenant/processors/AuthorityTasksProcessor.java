...
import org.example.integration.tenant.models.AuthorityCrmResponse;
import org.example.integration.tenant.models.AuthorityTaskConfiguration;
...
public class AuthorityTasksProcessor implements Runnable {
    ...
    public void run() {
        ...
        try {
            lastRunTime = S3Operator.getLastRuntime(trackId, TenantConstants.AUTHORITY_LAST_RUNTIME);
            if (lastRunTime == null) {
            ...
            getTaskList(trackId);
            S3Operator.updateLastRuntime(trackId,nextRunTime,TenantConstants.AUTHORITY_LAST_RUNTIME);
    ...
    public void getTaskList(String trackId) throws Exception {
        ...
        while (previouseResultCount == 100) {
            ...
            AuthorityTaskResponse taskResponse = AuthorityManager.getAuthorityTask(trackId, authorityToken, lastRunTime, skip, TenantConstants.TENANT_INTEGRATION_ID_CIVICA_TO_ASSETIC);
    ...
    public Map<String, Object> checkCategory(AuthorityTaskResponse.Result task,
        ...
        AuthorityCrmResponse.ResultItem crm = AuthorityManager.getCrmByFormattedAccount( trackId,task.getFormattedAccount(), authorityToken, TenantConstants.TENANT_INTEGRATION_ID_CIVICA_TO_ASSETIC);
    ...
    public DynamoData isProcessable(String sourceId, String trackId) {
        ...
        if (response.items().size() == 1) {
            ...
            if (data.getStatus().equalsIgnoreCase(TenantConstants.DYNAMO_STATUS_SUCCESS)) {
                // Item is already in progress
                log.info("{} Item is a success entry: {}",trackId,sourceId);
                return data;
            } else if (data.getStatus().equalsIgnoreCase(TenantConstants.DYNAMO_STATUS_RECEIVED) ||
